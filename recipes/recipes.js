...
function card(randomRecipes) {
    ...
    randomRecipes.forEach(randomRecipe => {
        ...
        recipeImage.alt = "Apple Crisp in a white bowl";
        ...
        randomRecipe.tags.forEach(tag => {
            const tagElement = document.createElement("span");
            tagElement.classList.add("tag");
            tagElement.textContent = tag;
            tagContainer.appendChild(tagElement);
        });
        recipeContent.appendChild(tagContainer);

        const titleElement = document.createElement("h2");
        titleElement.id = "recipeTitle";
        titleElement.textContent = randomRecipe.name;
        recipeContent.appendChild(titleElement);

        const ratingElement = document.createElement("span");
        ratingElement.classList.add("rating");
        ratingElement.setAttribute("role", "img");
        ratingElement.setAttribute("aria-label", `Rating: ${randomRecipe.rating} out of 5 stars`);
        for (let i = 0; i < 5; i++) {
            const star = document.createElement("span");
            star.classList.add("icon-star");
            if (i < randomRecipe.rating) {
                star.textContent = "⭐";
            } else {
                star.textContent = "☆";
            }
        ...
        recipeContent.appendChild(ratingElement);

        const descriptionElement = document.createElement("p");
        descriptionElement.classList.add("description");
        descriptionElement.textContent = randomRecipe.description;
        recipeContent.appendChild(descriptionElement);
...
searchBtn.addEventListener("click", () => {
    ...
    if (filteredRecipes.length > 0) {
        card(filteredRecipes);
    } else {
        alert("No recipes found.");
    }
});
const randomRecipe = getRandom();
card([randomRecipe]);
